...
exports.babyJub = exports.BabyJub = void 0;
...
class BabyJub {
    constructor(F) {
        ...
        this.pm1d2 = ff_1.Scalar.div(ff_1.Scalar.sub(this.p, ff_1.Scalar.one), 2n);
        this.Generator = [
        ...
        this.order = BigInt('21888242871839275222246405745257275088614511777268538073601725287587578984328');
        this.subOrder = ff_1.Scalar.shiftRight(this.order, 3n);
        this.A = F.e('168700');
        this.D = F.e('168696');
    }
    addPoint(a, b) {
    ...
    mulPointEscalar(base, e) {
        ...
        let rem = e;
        let exp = base;
        while (!ff_1.Scalar.isZero(rem)) {
            if (ff_1.Scalar.isOdd(rem)) {
                res = this.addPoint(res, exp);
            }
            exp = this.addPoint(exp, exp);
            rem = ff_1.Scalar.shiftRight(rem, ff_1.Scalar.one);
        }
        return res;
    }
    inSubgroup(P) {
    ...
    packPoint(P) {
        ...
        if (ff_1.Scalar.gt(n, this.pm1d2)) {
    ...
    unpackPoint(buff) {
        ...
        if (ff_1.Scalar.gt(F.toObject(P[1]), this.p))
            return null;
        const y2 = F.square(P[1]);
...
const F = new ff_1.F1Field(BigInt('21888242871839275222246405745257275088548364400416034343698204186575808495617'));
exports.babyJub = new BabyJub(F);
