...
class Parser {
    constructor(input) {
        this.input = input;
        this.currentIndex = 0; // index in the input which the parser is currently processing:
        this.out = { ...types_1.initDIDParams }; // the output DID that the parser will assemble as it steps through its state machine  // an error in the parser state machine
    }
    checkLength() {
    ...
    parseMethod() {
        ...
        for (;;) {
            ...
            if (utils_1.StringUtils.isNotDigit(char) && utils_1.StringUtils.isNotSmallLetter(char)) {
    ...
    parseId() {
        ...
        for (;;) {
            ...
            // idchar = ALPHA / DIGIT / "." / "-"
            if (utils_1.StringUtils.isNotValidIDChar(char)) {
    ...
    parseParamName() {
        ...
        this.out.params = [...this.out.params, new types_1.Param(input.slice(startIndex, currentIndex), '')];
        // return the next parser step
        return next ? next.bind(this) : null;
    }
    parseParamValue() {
        ...
        for (;;) {
            ...
            if (char == '%') {
                ...
                if (currentIndex + 2 >= inputLength ||
                    utils_1.StringUtils.isNotHexDigit(input[currentIndex + 1]) ||
                    utils_1.StringUtils.isNotHexDigit(input[currentIndex + 2])) {
            ...
            // idchar = ALPHA / DIGIT / "." / "-"
            if (!percentEncoded && utils_1.StringUtils.isNotValidParamChar(char)) {
    ...
    parsePath() {
        ...
        for (;;) {
            ...
            if (!percentEncoded && utils_1.StringUtils.isNotValidPathChar(char)) {
    ...
    parseQuery() {
        ...
        for (;;) {
            ...
            if (!percentEncoded && utils_1.StringUtils.isNotValidQueryOrFragmentChar(char)) {
